...
procedure TMTS.Play(TestMode: Boolean);
...
begin
  ...
  With FSupportS do begin
    ...
    Key.BorderColor:= clRed;
    Key.Cursor:= Self.Cursor;
    Key.Parent:= Self;
    Key.OnResponse:= KeySResponse;
    ...
    Key.SchMan.Kind:= FCfgTrial.SList.Values['SSch'];
...
procedure TMTS.TimerLatModTimer(Sender: TObject);
begin
  Inc(FLatMod);
end;

procedure TMTS.TimerLatCmpTimer(Sender: TObject);
begin
  Inc(FLatCmp);
end;
...
procedure TMTS.KeySResponse(Sender: TObject);
begin
  FTimerLatMod.Enabled:= False;
...
procedure TMTS.KeyCResponse(Sender: TObject);
begin
